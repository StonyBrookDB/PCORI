...
class LSTMSeqClassifier(nn.Module):
    ...
    Input (B, T, D) and classify the last hidden state.
    Provides sklearn-style APIs: 
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        x_tensor = torch.from_numpy(X).to(param_dev)
    ...
    def load_checkpoint(path: str, map_location=None):
        ...
        model = LSTMSeqClassifier(**ckpt["init_args"])
