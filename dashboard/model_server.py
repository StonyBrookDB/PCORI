...
'''
This API endpoint calls LLAMA Server with a Prompt containing all details and receives a Response 
Initiate Llama Server by running setup_server at "/home/kavya/MODELS/Llama-3.1-8B-Instruct".
After the Llama server is running, this API call will be ready to use.
'''
def get_llama_response(patient_id, risk_score, risk_level, top_features):
    features_text = "\n".join(
        [f"{f['ranking']}: score {f['score']}" for f in top_features]
    )
    prompt = f"""
        ...
        Explain in 5 lines in detail why the patient might be at {risk_level} risk level based on these features.
    """

    try:
        response = requests.post(
            "http://127.0.0.1:8765/llama",
            json={"content": prompt},
            timeout=300
        )
        ...
        return result.get("llm_reasoning", "Llama LLM reasoning unavailable.")
    except Exception as e:
        return f"Error getting reasoning: {e}"


@app.route("/risk_score-llama", methods=["POST"])
def get_risk_score_llama():
    try:
        patient_id = request.json.get("patient_id")
        ...
        llm_reasoning = get_llama_response(patient_id, prediction_prob, risk_level, top_features)
        return jsonify({
            "risk_score": round(float(prediction_prob), 2),
            "risk_level": risk_level,
            "explanation": explanation,
            "top_features": top_features,
            "llm_llama_reasoning": llm_reasoning
        })
